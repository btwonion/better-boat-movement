...
- Automatic boosts are rejected when the measured obstacle height exceeds the rise predicted from the `stepHeight` vertical velocity under the boat's gravity, plus `heightTolerance`. `heightTolerance` defaults to `0.25` blocks and permits small clearance differences from partial collision shapes and hull positioning.
